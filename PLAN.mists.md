...
- [ ] Add a local Mists interaction-reference checker that requires every `Covered` or `Mists-specific` row to cite at least one `tests/mists_*.rs` reference unless the row documents a non-Mists shared test exception.
