...
- [ ] Review shared C API changes under `src/c_api/`: integrated gate passes by moving `legacy_spell_book`, legacy specialization global registrations, legacy `GetSpellInfo`/`GetSpellTexture` globals, and Mists token-auction methods behind `client-mists`; continue reviewing remaining shared C API model changes, including PaperDoll, Currency, and SpellBook method additions.
