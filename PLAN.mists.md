...
- [ ] Rebuild the Mists GUI runner into a bounded target/cache location after the oversized `target/` cleanup, rerun the installed-addon Mists panel matrix from the first unproven addon (`BlizzMove`), and fix or promote the first real panel failure.
