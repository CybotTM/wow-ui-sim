...
- [ ] Re-run a bounded local installed-addon panel sample with a non-frame-mover UI addon after the legacy `CastSpell(slot, bookType)` fix, covering Spellbook/professions and TradeSkill with normal SavedVariables, and fix or promote the first addon-specific regression.
