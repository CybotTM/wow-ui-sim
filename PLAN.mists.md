...
- [ ] Fix `scripts/test-mists-addon-panels.sh` so a successful full-addon sweep does not exit through the `interrupted` cleanup path after retaining complete artifacts; then rerun the installed-addon Mists panel matrix from the first unproven addon.
