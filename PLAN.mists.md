...
- [ ] Review shared Lua API changes under `src/lua_api/`: integrated gate passes by moving `auction_verbs`, `bank_storage_verbs`, legacy profession/trainer registrations, rollout-added temporary Blizzard source patches, the Mists main-menu microbutton mouse-script suppression, and newly-added legacy action-bar boolean globals behind `client-mists`; continue reviewing remaining shared Lua changes, including `C_QuestChoice`, PVP honor/battleground globals and `PvpHonorState`, legacy spell-tab/skill-line return shapes, `action_bar_api` special-bar index behavior, and the shared MapCanvas scroll/zoom workaround.
