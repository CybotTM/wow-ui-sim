...
- [ ] Compare the Mists panel parity matrix against retail `PLAN.tests.md` Blizzard UI module coverage, identify any Mists-applicable user-facing Blizzard addon or workflow that still lacks a `client-mists` panel/interaction row, and promote the first real gap to a focused test/fix task.
