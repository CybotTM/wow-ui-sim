...
- [ ] Review test churn outside `tests/mists_*`: gated classic professions layout, legacy profession spellbook shape, challenge-mode Mists map, legacy ranged/weapon stat, and legacy quest POI table assertions behind `client-mists`; renamed misleading Mists map test to a seeded-world-map test; fixed Mists test compile dependencies on local `Interface/BlizzardUI` files; retail and Mists `cargo check --tests` pass; remaining follow-up is splitting oversized `src/iced_app/mouse_tests.rs` if we keep adding hit-test coverage there.
- [ ] Review scripts, CI, docs, and baseline artifacts outside Mists-specific paths: narrowed addon harness scope to Mists by removing the separate addon-harness workflow, dropping Wrath/Era manifest rows and the Bartender seed shim, rewriting the addon target docs, and pruning non-Mists/startup/perf baseline artifacts; continue reviewing remaining scripts/docs.
