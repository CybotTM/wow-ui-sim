...
| Interface options | Pass | screenshot: `target/mists-final-local-audit/panel-parity-with-saved-vars/interface-options/screenshot.webp`; dump: `target/mists-final-local-audit/panel-parity-with-saved-vars/interface-options/dump-tree.txt` | Mists 5.5.4 now includes `Blizzard_EditMode`; recapture this proof against the current Settings/EditMode flow. |
