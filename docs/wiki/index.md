...
| Page | Summary |
|------|---------|
| [[patch-12-0-0-api-audit]] | 12.0.0 wowless snapshot audit: 40 best-effort rows (including the bounded C_Timer NewTimer/NewTicker and C_ColorUtil behavioral slices), 70 evidence-required unsafe rows including 23 unresolved curve contracts, 8 unpublished C_StringUtil functions, five C_ColorUtil conversion/wrapper rows, and C_Timer.After without executable proof, plus the 12 C_Spell duration/metadata/boolean contracts without focused proof, 3300 untriaged occurrences including 17 curve metadata rows, 2554 added / 313 changed / 543 removed; no historical FrameXML or live-runtime claim |
