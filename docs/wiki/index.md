...
| [[world-map-texture-loading-budget]] | World map tile uploads were hidden in BC texture work; retained-path follow-up fixed cached-request selection, dirty-sentinel clobbering, inverted request priority, and the staged-vs-ready texture state bug; current live trace recapture confirms timer-path `RedrawAll` reaches the post-warmup `draw -> prepare -> present` sequence while atlas-ready counts rise and `textures_pending` stays true until `prepare()` drains the backlog |
