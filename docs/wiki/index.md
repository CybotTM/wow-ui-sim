...
| [[world-map-texture-loading-budget]] | World map tile uploads were hidden in BC texture work; retained-path follow-up fixed cached-request selection, dirty-sentinel clobbering, inverted request priority, and the staged-vs-ready texture state bug; current live trace recapture confirms timer-path `RedrawAll` reaches the post-warmup `draw -> prepare -> present` sequence, and the atlas-pressure audit found zero RGBA/BC upload rejections while atlas-ready counts rose and `textures_pending` stayed true until `prepare()` drained the backlog |
