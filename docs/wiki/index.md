...
| Page | Summary |
|------|---------|
| [[patch-12-0-0-api-audit]] | 12.0.0 wowless snapshot audit: 60 best-effort rows including bounded C_Timer, C_ColorUtil, C_TradeSkillUI, and C_DamageMeter behavioral slices; 91 evidence-required unsafe rows; 3259 untriaged occurrences. C_DamageMeter has exactly 19 best-effort behavioral rows limited to exact seeded/empty/shape/type/lookup assertions; 10 seeded-but-unasserted/reset rows remain evidence-required with no approval path; six metadata-only structure rows remain untriaged. Schema provenance only: no historical FrameXML or live runtime behavior is claimed, and the audit remains open. |
