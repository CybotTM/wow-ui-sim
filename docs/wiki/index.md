...
| [[mouse-dead-probe-blockers-idle-ticks]] | Mouse input dead at "50 FPS": CoreBehaviorProbe left full-screen DIALOG mouse blockers (its C_Timer cleanup chain stalls because pending timers don't wake the idle tick loop — OPEN); loader now refuses TOCs that don't name their folder, so `.disabled` renames disable addons |
...
| [[mists-world-map-startup]] | Mists startup WorldMap/Leatrix failures came from missing Mists WorldMap cache files plus narrow Cata/Mists map-provider compatibility gaps |
