...
Clearing the aborts let execution reach code that had never run, which surfaced ten further defects. Each is its own commit and each was measured, not inferred.
...
**The brightness lift is not a defect, and neither is its absence.** With `WOW_SIM_BRIGHTNESS_BOOST=0` the simulator reproduces atlas texels 1:1 — the action bar's gryphon end cap, aligned pixel-exactly in a client capture and a simulator render by masked FFT correlation (both at (1085,1236) on a 3440×1440 canvas at 1.6875), renders source luminance 50 → 53, 90 → 90, 145 → 139. The client capture shows the same texels at 93, 136 and 194: a near-constant offset of about +45 across all bins, and the nominal yellow of `NORMAL_FONT_COLOR` (255, 209, 0) appears as (255, 255, 31) in the same capture, which is the same offset again. That is a whole-frame brightness offset from the client's display settings, not a property of the UI. The historical `pow(rgb, 1/1.5)` lift happens to land within 4 luminance points of that capture in the midtones and 25 short in the highlights; it stays the default as an on-screen aid, and a capture meant to be compared texel-for-texel wants it off.
