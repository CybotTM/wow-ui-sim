...
Fix: successful recipe crafts now start a player cast using the recipe ID/name, fire `UNIT_SPELLCAST_START` for `"player"`, and emit `UPDATE_TRADESKILL_CAST_STOPPED` when that cast completes. Inventory mutation remains immediate for now, preserving existing crafting tests.

Regression coverage lives in `tests/test_crafting.rs`: `craft_recipe_starts_player_cast_for_cast_bar` asserts both `UnitCastingInfo("player")` and the start event.
