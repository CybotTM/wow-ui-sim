...
2. **Minimap method removals:** `SetBlipTexture`, `SetCorpsePOIArrowTexture`, `SetIconTexture`, `SetPOIArrowTexture`, `SetPlayerTexture`, `SetStaticPOIArrowTexture`. Removing currently registered methods risks breaking cached/vendor UI; remains an approved exception pending current-source proof.
3. **Secret/aspect widget changes:** focused 12.0.7 proof verifies the four Button methods, four ScrollFrame methods, five `SetFont` methods, and all six proposed removed Minimap texture setters remain callable compatibility functions. Exact secret/aspect/asset semantics remain best-effort. `ModelSceneActorBase:GetModelUnitGUID` is absent and falls under the intentional permanent no-3D gap; it is retained as an explicit exception candidate pending the final itemized approval request.
