...
| FrameXML symbol snapshot | Full 320-added/112-removed inventory is itemized in [[patch-12-1-framexml-symbol-inventory]]. Current classification: 1 implemented, 18 best-effort, and 413 exception-requested pending strict re-triage. The five `DifficultyUtil` color helpers are dynamic delegates; `GetTimeStringFromSeconds` is cross-flavor contamination; AuctionHouse/GuildBank/Garrison hide wrappers are source/runtime mismatches; ItemUpgrade and BlackMarket hide entries are reversed-name mismatches. All addon-owned mismatch claims use explicit PTR LoD loads. |
