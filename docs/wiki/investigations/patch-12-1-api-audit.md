...
| FrameXML symbol snapshot | Full 320-added/112-removed inventory is itemized in [[patch-12-1-framexml-symbol-inventory]]. Current classification: 1 implemented, 15 best-effort, and 416 exception-requested pending strict re-triage. The five `DifficultyUtil` color helpers are 12.1 post-load dynamic delegates; `GetTimeStringFromSeconds` is cross-flavor contamination; AuctionHouse/GuildBank hide wrappers are source/runtime mismatches; `HideItemUpgradeFrame` is a reversed-name mismatch because PTR exposes `ItemUpgradeFrame_Hide`; focused proof explicitly loads `Blizzard_ItemUpgradeUI` before checking the reversed name remains nil. |
