...
- pending commit — modeled Battle.net appear-offline intent on `SimState.bnet_appear_offline` and moved the title-friend unit invite probe out of Lua inert defaults.
...
- **Housing availability service results** — `C_HousingBlueprint.GetExportAvailability`, `GetFeatureAvailability`, and `GetImportAvailability` are still compatibility nil returns. They need PTR/service payload captures for result enums, disabled reasons, and current-location behavior before they can become modeled behavior. Exception request pending; do not count 12.1 complete until approved or implemented.
- **Changed structure payloads with real data** — inert compatibility fields were added where safe, and Battle.net title-friend custom names/tags, pending Battle.net invites, Encounter Journal difficulty helpers, Discord enabled-state, and local housing state now have best-effort simulator models. Exact payloads for Discord server/channel data, housing service collections/availability, cooldown viewer, pet journal, LFG, player choice, tiered entrance, and private aura structures require backing models or live captures before claiming behavioral fidelity.
