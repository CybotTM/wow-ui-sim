...
| FrameXML symbol snapshot | Full 320-added/112-removed inventory is itemized in [[patch-12-1-framexml-symbol-inventory]]. Current classification: 1 implemented, 12 best-effort, and 419 exception-requested pending strict re-triage. The five `DifficultyUtil` color helpers are 12.1 post-load dynamic delegates; `GetTimeStringFromSeconds` is documented cross-flavor snapshot contamination and verified absent from PTR mainline at every startup phase. |
