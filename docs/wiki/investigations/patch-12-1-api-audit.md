...
- pending commit — modeled `C_Discord.IsEnabled` from the existing `discordClientEnabled` CVar while leaving the rest of Discord as inert service placeholders.
...
| Pending Battle.net title-friend invites / housing blueprint/editor APIs | Present as inert compatibility defaults unless backed by existing simulator state. These are callable stubs, not real service models. |
