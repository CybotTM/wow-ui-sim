...
| FrameXML symbol snapshot | Full 320-added/112-removed inventory is itemized in [[patch-12-1-framexml-symbol-inventory]]. Current classification: 1 implemented, 16 best-effort, and 415 exception-requested pending strict re-triage. The five `DifficultyUtil` color helpers are 12.1 post-load dynamic delegates; `GetTimeStringFromSeconds` is cross-flavor contamination; AuctionHouse/GuildBank hide wrappers are source/runtime mismatches; ItemUpgrade and BlackMarket hide entries are reversed-name mismatches proven after explicit PTR LoD loads. |
