The two retail 12.0.0 removed docs-extra API rows `docs.extra_apis.ShowCloak` and `docs.extra_apis.ShowHelm` are **evidence-required/unsafe**. Source removal and absent checked-in registrations do not prove full-LoD runtime absence or replacement semantics; required probes must distinguish the removed docs-extra entries from the separate same-name added APIs. Claims are bounded to source removal only. Current totals are **1919 best-effort, 1027 evidence-required, 2 exception-requested, and 462 untriaged rows** (3410 total).
