...
Added `investigations/blizzard-ui-manifest-drift.md`. `data/blizzard-ui-files/ptr.txt` had drifted to 310 addon directories against the 348 the Gethe `ptr` branch ships, contradicting the "full file list, no filtering" contract its own generator documents. The 39 unsynced directories include `Blizzard_ManagedFrameSystem` (a declared dependency of twelve synced TOCs), `Blizzard_StatusTrayManager` and `Blizzard_GameMenuEsc`; the last defines `RegisterGameMenuEscHandler`, which 25 Blizzard files call at file scope, so each of them lost every definition below the call — `EditModeManager.lua` before `RegisterSystemFrame`, which in turn aborted `OnSystemLoad` for every `EditModeSystemTemplate` frame. Regenerating the manifest and the bundled listfile subset, plus thirteen simulator-owned enum tables that only became reachable once the aborts cleared, drops startup errors on `client-ptr` from 659 to 237 and distinct messages from 56 to 12. The managed-frame containers and the edit-mode system registration both start working as a result. Two refuted leads are recorded on the page: the ptr cache was not stale, and importing the real `_retail_/WTF` layout makes the HUD worse rather than better.
