...
**Versioned UI strings** — `register_all_ui_strings()` combines generated global strings with static compatibility data. The enUS retail 12.1 slice registers 45 exact strings only under `profile-retail` + `retail-12-1-0`: the original probe-backed values plus 13 housing Settings labels/tooltips pinned from build `12.1.0.69497` by `cc02aa287`. Those values let the canonical Interface Settings registrant complete; without them it assigned category ID 7 and aborted before registration. The 12 probe-proven nil globals remain absent rather than receiving placeholders or aliases. Housing service/state behavior remains unmodeled.
