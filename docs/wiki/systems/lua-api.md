...
**Versioned UI strings** — `register_all_ui_strings()` combines generated global strings with static compatibility data. The retail 12.1 table registers 61 strings only under `profile-retail` + `retail-12-1-0`: 45 existing strings plus 16 retail-12.1-gated Social UI compatibility labels from `cfec8576b`. The 16 English label values were not captured by the existing 49-candidate live probe; they keep `Blizzard_SocialUIShared` tag/presence tables populated during preload. `cfec8576b` also publishes `Enum.SocialUIPresenceType` (`Unknown=0` through `AppearOffline=5`) and metadata (`0..5`, six values); `8f070a410` publishes `Enum.SocialUIBlockType` (`None=0`, `Ignore=1`, `BattleNetInviteBlock=2`) and metadata (`0..2`, three values). The 12 probe-proven nil globals remain absent rather than receiving placeholders or aliases. Housing and Social UI service/state behavior remains unmodeled.
