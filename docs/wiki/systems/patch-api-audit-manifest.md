...
- `data/patch-api/sources/12.1-behaviors.json`, `data/patch-api/12.1-behaviors.json`, `docs/generated/patch-12-1-behavior-checklist.md`, and [[patch-12-1-behavior-inventory]] preserve 54 independently testable non-FrameXML behavior boundaries. Thirty direct-test-backed rows are best-effort and 24 are item-specific exception requests; candidate disposition is 30 safe best-effort, 21 unsafe, and 3 impossible, with no exception approved.
...
- `data/patch-api/sources/12.0.5-probes.json`, `data/patch-api/12.0.5-probes.json`, `docs/generated/patch-12-0-5-probe-checklist.md`, and [[patch-12-0-5-probe-inventory]] preserve 38 probe subfindings. Thirty-two rows have direct best-effort behavioral evidence and 6 remain untriaged; prior documented states remain separate from machine status.
...
Current 12.1 FrameXML totals are **1 implemented, 431 best-effort, 0 exception-requested, and 0 untriaged**. The separate broader behavior register is **0 implemented, 30 best-effort, 24 exception-requested, and 0 untriaged**. Zero safe-best-effort rows remain; the full 12.1 objective remains open only for the 24 unsafe/impossible approval decisions, none of which is approved.
