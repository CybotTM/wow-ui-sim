...
- `data/patch-api/sources/12.0.5-probes.json`, `data/patch-api/12.0.5-probes.json`, `docs/generated/patch-12-0-5-probe-checklist.md`, and [[patch-12-0-5-probe-inventory]] preserve 38 probe subfindings: 33 best-effort, 5 exception-requested, and 0 untriaged, with no approvals. The five exceptions remain item-specific: same-size window transitions are impossible at the current input boundary; XML frame-level raw provenance is unavailable; Store protection, dropdown population, and forbidden-descendant behavior are unsafe pending live evidence.
