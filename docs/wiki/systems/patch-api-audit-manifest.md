...
- The bounded 12.0.0 `C_NamePlate` slice classifies six added 2D size/hit-test/state APIs as evidence-required/unsafe using checked-in source-register evidence and the permanent `src/c_api/permanent_shims/c_nameplate.rs` shim; `C_NamePlateManager.IsNamePlateUnitBehindCamera` is exception-requested/impossible under the already-decided `AGENTS.md#intentional-gaps` permanent no-3D scope, not a user approval request, and the 19 removed C_NamePlate rows remain untriaged.
...
- The bounded 12.0.0 `C_TransmogCollection` slice classifies exactly 23 added/changed custom-set and appearance-source rows as evidence-required/unsafe using checked-in source-register evidence, the seeded/partial `src/lua_api/globals/missing_surface/transmog_collection.rs` surface, and `src/lua_api/state_types/collections.rs`; all 10 removed outfit rows remain untriaged. Custom-set lifecycle, hyperlinks, persistence, validation, and exact `TransmogAppearanceSourceInfoData` semantics remain unproven, related local tests do not establish authoritative retail 12.0.0 behavior, and no approval can close these rows.
