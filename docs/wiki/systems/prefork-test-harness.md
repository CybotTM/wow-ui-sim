...
The generated ordinary full-UI migration aggregate is now 1,591 cases: 1,480 previously migrated cases, 15 SpellSearch cases, and 96 post-start explicit/LoadOnDemand cases across ten modules. SpellSearch cases explicitly load `Blizzard_SpellSearch` in each child after the normal preload; the other 96 use borrowed-environment child setup while preserving explicit dependency order. Generated-registry conformance totals 111 cases including SpellSearch; the `is_addon_loaded` behavior filter passed 106/106, with no orphan processes. The nine manual keybinding cases, two behavioral-messaging cases, and one nested registry/preloaded-startup fixture remain separately categorized. Custom fixtures with pre-start or otherwise non-equivalent lifecycle ordering remain excluded pending separate behavioral-equivalence proof.
