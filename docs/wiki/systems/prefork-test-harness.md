...
The generated ordinary full-UI migration aggregate is now 1,780 cases: 1,591 previously migrated cases plus 189 post-start housing LoadOnDemand cases across 13 modules. The housing cases use borrowed-environment child setup that performs only the original explicit addon loads after the shared normal-retail preload, preserving dependency order; they do not alter the parent preload. Generated-registry conformance for the housing batch totals 189 cases; the representative `is_addon_loaded` behavior filter passed 119/119, with no orphan processes. The nine manual keybinding cases, two behavioral-messaging cases, one nested registry/preloaded-startup fixture, and earlier SpellSearch/explicit batches remain separately categorized. Pre-start, partial/custom, glue, and otherwise non-equivalent lifecycle fixtures remain excluded from the normal-preload migration.
