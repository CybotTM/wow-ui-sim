...
pub fn denormalize(data: &AllData) -> (Vec<TreeInfo>, Vec<NodeInfo>) {
    ...
    let node_infos: Vec<NodeInfo> = data
        .nodes
        .iter()
        .map(|n| build_node_info(n, &data.joins, &edges_by_target))
        .collect();
...
pub fn write_header(out: &mut File) -> std::io::Result<()> {
    writeln!(
        out,
        "//! Auto-generated trait/talent data from WoW CSV exports."
    )?;
    writeln!(
        out,
        "//! Do not edit manually - regenerate with: wow-cli generate traits"
    )?;
    writeln!(out)?;
    Ok(())
}
...
fn write_trait_node_info_struct(out: &mut File) -> std::io::Result<()> {
    write_struct(
        ...
        &[
            ...
            "pub flags: u32",
            "pub sub_tree_id: u32",
            "pub entry_ids: &'static [u32]",
            "pub cond_ids: &'static [u32]",
            "pub edges: &'static [TraitEdgeInfo]",
...
pub fn write_tree_db(out: &mut File, trees: &[TreeInfo]) -> Result<(), Box<dyn std::error::Error>> {
    for t in trees {
    ...
    writeln!(
        out,
        "pub static TRAIT_TREE_DB: phf::Map<u32, TraitTreeInfo> = {};",
        builder.build()
    )?;
    writeln!(out)?;
...
pub fn write_node_db(out: &mut File, nodes: &[NodeInfo]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    writeln!(
        out,
        "pub static TRAIT_NODE_DB: phf::Map<u32, TraitNodeInfo> = {};",
        builder.build()
    )?;
    writeln!(out)?;
...
pub fn write_entry_db(
    ...
    writeln!(
        out,
        "pub static TRAIT_ENTRY_DB: phf::Map<u32, TraitEntryInfo> = {};",
        builder.build()
    )?;
    writeln!(out)?;
...
pub fn write_definition_db(
    ...
    for d in defs {
        let value = format!(
            ...
            d.spell_id,
            d.override_icon,
            d.overrides_spell_id,
            d.visible_spell_id,
            escape_str(&d.override_name),
            escape_str(&d.override_subtext),
    ...
    writeln!(
        out,
        "pub static TRAIT_DEFINITION_DB: phf::Map<u32, TraitDefInfo> = {};",
        builder.build()
    )?;
    writeln!(out)?;
...
pub fn write_cond_db(out: &mut File, conds: &[RawCond]) -> Result<(), Box<dyn std::error::Error>> {
    let mut builder = phf_codegen::Map::new();
    for c in conds {
        let value = format!(
    ...
    writeln!(
        out,
        "pub static TRAIT_COND_DB: phf::Map<u32, TraitCondInfo> = {};",
        builder.build()
    )?;
    writeln!(out)?;
...
pub fn write_sub_tree_db(
    ...
    writeln!(
        out,
        "pub static TRAIT_SUBTREE_DB: phf::Map<u32, TraitSubTreeInfo> = {};",
        builder.build()
    )?;
    writeln!(out)?;
...
pub fn write_currency_db(
    ...
    writeln!(
        out,
        "pub static TRAIT_CURRENCY_DB: phf::Map<u32, TraitCurrencyInfo> = {};",
        builder.build()
    )?;
    writeln!(out)?;
...
pub fn write_tests(out: &mut File) -> std::io::Result<()> {
    ...
    writeln!(
        out,
        "        let tree = TRAIT_TREE_DB.get(&790).expect(\"tree 790\");"
    )?;
    ...
    writeln!(
        out,
        "        let node = TRAIT_NODE_DB.get(&first_node_id).expect(\"first node\");"
    )?;
...
fn emit_edge_array(out: &mut File, name: &str, edges: &[(u32, u32, u32)]) -> std::io::Result<()> {
    ...
    for (i, (src, etype, vstyle)) in edges.iter().enumerate() {
        ...
        write!(
            out,
            "TraitEdgeInfo {{ source_node_id: {}, edge_type: {}, visual_style: {} }}",
            src, etype, vstyle
        )?;
    }
    writeln!(out, "];")?;
