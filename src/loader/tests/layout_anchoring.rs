...
fn test_anchor_initial_state() {
    let (t, _) = load_test_lua("layout-anchor-init", r#"
...
fn test_set_point_center_defaults_to_parent() {
    let (t, _) = load_test_lua("layout-sp-center", r#"
...
fn test_set_point_with_offset() {
    let (t, _) = load_test_lua("layout-sp-offset", r#"
...
fn test_set_point_full_form() {
    let (t, _) = load_test_lua("layout-sp-full", r#"
        local f = CreateFrame("Frame", nil, UIParent)
        f:SetSize(50, 30)
...
fn test_set_point_with_string_name() {
    let (t, _) = load_test_lua("layout-sp-string", r#"
...
fn test_set_point_replaces_same_point() {
    let (t, _) = load_test_lua("layout-sp-replace", r#"
        local f = CreateFrame("Frame", nil, UIParent)
        f:SetSize(50, 30)
...
fn test_get_num_points_increments() {
    let (t, _) = load_test_lua("layout-numpoints", r#"
        local f = CreateFrame("Frame", nil, UIParent)
        N0 = f:GetNumPoints()
        f:SetPoint("TOPLEFT")
        N1 = f:GetNumPoints()
...
fn test_get_point_sorted_by_sort_key() {
    let (t, _) = load_test_lua("layout-gp-sorted", r#"
        local f = CreateFrame("Frame", nil, UIParent)
...
fn test_get_point_out_of_range() {
    let (t, _) = load_test_lua("layout-gp-oor", r#"
        local f = CreateFrame("Frame", nil, UIParent)
        f:SetPoint("CENTER")
...
fn test_get_point_default_index() {
    let (t, _) = load_test_lua("layout-gp-default", r#"
        local f = CreateFrame("Frame", nil, UIParent)
        f:SetSize(50, 30)
        f:SetPoint("CENTER")
...
fn test_clear_all_points() {
    let (t, _) = load_test_lua("layout-clearall", r#"
        local f = CreateFrame("Frame", nil, UIParent)
...
fn test_clear_all_points_idempotent() {
    let (t, _) = load_test_lua("layout-clearall-idem", r#"
        local f = CreateFrame("Frame")
        f:ClearAllPoints()
        f:ClearAllPoints()
        N = f:GetNumPoints()
...
fn test_clear_all_points_invalidates_resolved_rect() {
    let (t, _) = load_test_lua("layout-clearall-dirty", r#"
        local f = CreateFrame("Frame", nil, UIParent)
        f:SetSize(50, 30)
        f:SetPoint("CENTER")
        f:GetLeft()
...
fn test_clear_point_specific() {
    let (t, _) = load_test_lua("layout-clearpoint", r#"
        local f = CreateFrame("Frame", nil, UIParent)
        f:SetPoint("TOPLEFT", UIParent, "TOPLEFT")
        f:SetPoint("BOTTOMRIGHT", UIParent, "BOTTOMRIGHT")
...
fn test_clear_point_nonexistent_is_silent() {
    let (t, _) = load_test_lua("layout-clearpoint-noop", r#"
        local f = CreateFrame("Frame", nil, UIParent)
        f:SetPoint("CENTER")
        f:ClearPoint("TOPLEFT")
        N = f:GetNumPoints()
...
fn test_set_all_points_creates_tl_br() {
    let (t, _) = load_test_lua("layout-sap-tlbr", r#"
        local f = CreateFrame("Frame", nil, UIParent)
        f:SetAllPoints()
        N = f:GetNumPoints()
        P1 = select(1, f:GetPoint(1))
...
fn test_set_all_points_explicit_target() {
    let (t, _) = load_test_lua("layout-sap-target", r#"
...
fn test_set_all_points_clears_previous() {
    let (t, _) = load_test_lua("layout-sap-clears", r#"
        local f = CreateFrame("Frame", nil, UIParent)
...
fn test_set_all_points_matches_parent_size() {
    let (t, _) = load_test_lua("layout-sap-size", r#"
...
fn test_adjust_points_offset_single() {
    let (t, _) = load_test_lua("layout-adjoffset", r#"
        local f = CreateFrame("Frame", nil, UIParent)
        f:SetSize(50, 30)
...
fn test_adjust_points_offset_all_anchors() {
    let (t, _) = load_test_lua("layout-adjoffset-all", r#"
        local f = CreateFrame("Frame", nil, UIParent)
        f:SetPoint("TOPLEFT", UIParent, "TOPLEFT", 10, -10)
...
fn test_anchor_cycle_detection() {
    let (t, _) = load_test_lua("layout-cycle", r#"
...
fn test_anchor_cycle_detection_chain() {
    let (t, _) = load_test_lua("layout-cycle-chain", r#"
...
fn test_set_point_dirties_resolved_rect() {
    let (t, _) = load_test_lua("layout-dirty-sp", r#"
        local f = CreateFrame("Frame", nil, UIParent)
        f:SetSize(50, 30)
        f:SetPoint("CENTER")
        f:GetLeft()
