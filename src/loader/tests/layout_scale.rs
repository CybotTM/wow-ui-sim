...
fn test_scale_defaults() {
    let (t, _) = load_test_lua("layout-scale-def", r#"
...
#[test]
fn test_set_scale() {
    let (t, _) = load_test_lua("layout-set-scale", r#"
...
fn test_set_scale_zero_errors() {
    let (t, _) = load_test_lua("layout-scale-zero", r#"
        local f = CreateFrame("Frame")
...
fn test_set_scale_negative_errors() {
    let (t, _) = load_test_lua("layout-scale-neg", r#"
        local f = CreateFrame("Frame")
...
fn test_set_scale_small_positive() {
    let (t, _) = load_test_lua("layout-scale-small", r#"
        local f = CreateFrame("Frame")
        f:SetScale(0.001)
        SCALE = f:GetScale()
    "#);
...
fn test_effective_scale_parent_child() {
    let (t, _) = load_test_lua("layout-effscale", r#"
...
fn test_effective_scale_three_levels() {
    let (t, _) = load_test_lua("layout-effscale3", r#"
...
fn test_set_scale_updates_child_effective() {
    let (t, _) = load_test_lua("layout-scale-update", r#"
        local parent = CreateFrame("Frame", nil, UIParent)
...
fn test_scale_does_not_affect_explicit_size() {
    let (t, _) = load_test_lua("layout-scale-size", r#"
        local f = CreateFrame("Frame", nil, UIParent)
        f:SetSize(100, 50)
...
fn test_scale_divides_position_queries() {
    let (t, _) = load_test_lua("layout-scale-leftright", r#"
        local f = CreateFrame("Frame", nil, UIParent)
        f:SetSize(100, 50)
...
fn test_scale_affects_get_rect() {
    let (t, _) = load_test_lua("layout-scale-rect", r#"
        local f = CreateFrame("Frame", nil, UIParent)
        f:SetSize(100, 50)
        f:SetPoint("TOPLEFT", UIParent, "TOPLEFT", 0, 0)
        f:SetScale(2.0)
