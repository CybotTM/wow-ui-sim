...
-- Game menu Escape handlers.
--
-- `RegisterGameMenuEscHandler` and `GameMenuEscPriority` are client-side
-- globals: nothing in the Blizzard UI defines them and Blizzard_APIDocumentation
-- carries no entry. Twenty-five Blizzard files call the function at FILE SCOPE,
-- so without it each of those files raises there and silently loses every
-- definition below that line. EditModeManager.lua is the costly one -- it aborts
-- before EditModeManagerFrameMixin:RegisterSystemFrame, so
-- EditModeSystemMixin:OnSystemLoad raises for every EditModeSystemTemplate frame
-- and those frames never finish their OnLoad.
--
-- Handlers are consulted in ascending priority until one returns true; the
-- client falls through to the ESCAPE keybinding (ToggleGameMenu) when none
-- does. The numeric values below are INFERRED from the member names and their
-- call sites, as Blizzard documents none: the relative order is the modelled
-- contract, the numbers themselves are arbitrary.
if rawget(_G, "GameMenuEscPriority") == nil then
  GameMenuEscPriority = {
    FrameworkPre = 1, -- HelpFrame, HouseEditor
    Framework = 2,    -- GameMenu, Settings, EditMode, TimeManager, ReportFrame
    Dialog = 3,       -- StaticPopup
    Menu = 4,         -- SpellFlyout, chat dock overflow
    AddOn = 5,        -- ordinary panels
    AddOnPost = 6,    -- CloseAllWindows catch-all
    AddOnPost2 = 7,   -- LootFrame, after the catch-all
  }
end

local __wow_esc_handlers = {}

if rawget(_G, "RegisterGameMenuEscHandler") == nil then
  function RegisterGameMenuEscHandler(priority, handler)
    if type(handler) ~= "function" then
      return
    end
    __wow_esc_handlers[#__wow_esc_handlers + 1] = {
      priority = tonumber(priority) or GameMenuEscPriority.AddOn,
      handler = handler,
      -- Registration order breaks ties, so handlers sharing a priority keep
      -- the order their files were loaded in.
      seq = #__wow_esc_handlers + 1,
    }
  end
end

-- Called by the simulator's ESCAPE dispatch before the keybinding runs.
-- Returns true when a handler consumed the key press.
if rawget(_G, "__wow_dispatch_game_menu_esc") == nil then
  function __wow_dispatch_game_menu_esc()
    local ordered = {}
    for index = 1, #__wow_esc_handlers do
      ordered[index] = __wow_esc_handlers[index]
    end
    ...
    for index = 1, #ordered do
      local ok, consumed = pcall(ordered[index].handler)
