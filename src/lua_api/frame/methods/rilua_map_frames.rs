...
use crate::lua_api::frame::methods::rilua_core_state;
...
fn get_ui_map_id(state: &mut LuaState) -> LuaResult<u32> {
    ...
    let map_id = {
        ...
        if is_fog_of_war_frame(&sim, id) {
            ...
            drop(sim);
            return rilua_core_state::get_ui_map_id(state);
        }
    };
...
fn set_ui_map_id(state: &mut LuaState) -> LuaResult<u32> {
    ...
    drop(sim);
    rilua_core_state::set_map_id(state)
}
    ...
    F: Fn(&crate::lua_api::state::FogOfWarFrameState) -> Option<String>,
{
    ...
    match atlas {
        Some(value) => {
            let value = crate::lua_api::rilua_methods::create_string(state, &value);
            state.push(value);
        }
...
fn get_mouse_over_units(state: &mut LuaState) -> LuaResult<u32> {
    ...
    for unit in units {
        let unit = crate::lua_api::rilua_methods::create_string(state, &unit);
        state.push(unit);
    }
    Ok(count)
}
