...
pub fn set_alpha_gradient(state: &mut LuaState) -> LuaResult<u32> {
    let id = frame_id_from_stack(state, 1)?;
    let start = match stack_val(state, 2) {
        Val::Num(value) => value as f32,
        _ => 0.0,
    };
    let length = match stack_val(state, 3) {
        Val::Num(value) => value as f32,
        Val::Table(point) => {
            let point = Val::Table(point);
            match table_get(state, point, "y") {
                Val::Num(value) => value as f32,
                _ => 1.0,
            }
        }
...
fn texture_arg(state: &LuaState, index: i32) -> Option<String> {
    val_to_string(state, stack_val(state, index))
}

fn number_arg(state: &LuaState, index: i32) -> Option<f64> {
...
fn bool_arg_opt(state: &LuaState, index: i32) -> Option<bool> {
    match stack_val(state, index) {
...
fn set_minimap_texture_field(
    state: &mut LuaState,
    write: impl FnOnce(&mut crate::widget::Frame, Option<String>),
) -> LuaResult<u32> {
    let id = frame_id_from_stack(state, 1)?;
    let asset = texture_arg(state, 2);
    let mut sim = borrow_state_mut(state)?;
    if let Some(frame) = sim.widgets.get_mut_visual(id) {
        write(frame, asset);
    }
...
pub fn set_blip_texture(state: &mut LuaState) -> LuaResult<u32> {
    set_minimap_texture_field(state, |frame, asset| frame.minimap_blip_texture = asset)
}

pub fn set_minimap_mask_texture(state: &mut LuaState) -> LuaResult<u32> {
    set_minimap_texture_field(state, |frame, asset| frame.minimap_mask_texture = asset)
}

pub fn set_icon_texture(state: &mut LuaState) -> LuaResult<u32> {
    set_minimap_texture_field(state, |frame, asset| frame.minimap_icon_texture = asset)
}

pub fn set_player_texture(state: &mut LuaState) -> LuaResult<u32> {
    set_minimap_texture_field(state, |frame, asset| frame.minimap_player_texture = asset)
}

pub fn set_poi_arrow_texture(state: &mut LuaState) -> LuaResult<u32> {
    set_minimap_texture_field(state, |frame, asset| {
        frame.minimap_poi_arrow_texture = asset
    })
}

pub fn set_corpse_poi_arrow_texture(state: &mut LuaState) -> LuaResult<u32> {
    set_minimap_texture_field(state, |frame, asset| {
        frame.minimap_corpse_poi_arrow_texture = asset
    })
}

pub fn set_static_poi_arrow_texture(state: &mut LuaState) -> LuaResult<u32> {
    set_minimap_texture_field(state, |frame, asset| {
        frame.minimap_static_poi_arrow_texture = asset
    })
}

pub fn set_zoom(state: &mut LuaState) -> LuaResult<u32> {
    let id = frame_id_from_stack(state, 1)?;
    let zoom = number_arg(state, 2).unwrap_or(0.0);
    let fields = get_or_create_frame_fields(state, id);
    table_set(state, fields, "__minimap_zoom", Val::Num(zoom));
    Ok(0)
}

pub fn get_zoom(state: &mut LuaState) -> LuaResult<u32> {
    let id = frame_id_from_stack(state, 1)?;
    let fields = get_or_create_frame_fields(state, id);
    let zoom = match table_get(state, fields, "__minimap_zoom") {
        Val::Num(value) => value,
        _ => 0.0,
    };
    state.push(Val::Num(zoom));
    Ok(1)
}

pub fn ping_location(state: &mut LuaState) -> LuaResult<u32> {
    let id = frame_id_from_stack(state, 1)?;
    let x = number_arg(state, 2).unwrap_or(0.0) as f32;
    let y = number_arg(state, 3).unwrap_or(0.0) as f32;
    let mut sim = borrow_state_mut(state)?;
    if let Some(frame) = sim.widgets.get_mut_visual(id) {
...
fn with_visual_frame(
    state: &mut LuaState,
    write: impl FnOnce(&mut crate::widget::Frame),
) -> LuaResult<u32> {
    let id = frame_id_from_stack(state, 1)?;
    let mut sim = borrow_state_mut(state)?;
    if let Some(frame) = sim.widgets.get_mut_visual(id) {
...
pub fn set_quest_blob_inside_texture(state: &mut LuaState) -> LuaResult<u32> {
    let asset = texture_arg(state, 2);
    with_visual_frame(state, |frame| frame.quest_blob_inside.texture = asset)
}
pub fn set_quest_blob_inside_alpha(state: &mut LuaState) -> LuaResult<u32> {
    let alpha = number_arg(state, 2).unwrap_or(0.0);
    with_visual_frame(state, |frame| frame.quest_blob_inside.alpha = alpha)
}
pub fn set_quest_blob_outside_texture(state: &mut LuaState) -> LuaResult<u32> {
    let asset = texture_arg(state, 2);
    with_visual_frame(state, |frame| frame.quest_blob_outside.texture = asset)
}
pub fn set_quest_blob_outside_alpha(state: &mut LuaState) -> LuaResult<u32> {
    let alpha = number_arg(state, 2).unwrap_or(0.0);
    with_visual_frame(state, |frame| frame.quest_blob_outside.alpha = alpha)
}
pub fn set_quest_blob_ring_texture(state: &mut LuaState) -> LuaResult<u32> {
    let asset = texture_arg(state, 2);
    with_visual_frame(state, |frame| frame.quest_blob_ring.texture = asset)
}
pub fn set_quest_blob_ring_alpha(state: &mut LuaState) -> LuaResult<u32> {
    let alpha = number_arg(state, 2).unwrap_or(0.0);
    with_visual_frame(state, |frame| frame.quest_blob_ring.alpha = alpha)
}
pub fn set_quest_blob_ring_scalar(state: &mut LuaState) -> LuaResult<u32> {
    let scalar = number_arg(state, 2).unwrap_or(0.0);
    with_visual_frame(state, |frame| frame.quest_blob_ring.scalar = scalar)
}
pub fn set_task_blob_inside_texture(state: &mut LuaState) -> LuaResult<u32> {
    let asset = texture_arg(state, 2);
    with_visual_frame(state, |frame| frame.task_blob_inside.texture = asset)
}
pub fn set_task_blob_inside_alpha(state: &mut LuaState) -> LuaResult<u32> {
    let alpha = number_arg(state, 2).unwrap_or(0.0);
    with_visual_frame(state, |frame| frame.task_blob_inside.alpha = alpha)
}
pub fn set_task_blob_outside_texture(state: &mut LuaState) -> LuaResult<u32> {
    let asset = texture_arg(state, 2);
    with_visual_frame(state, |frame| frame.task_blob_outside.texture = asset)
}
pub fn set_task_blob_outside_alpha(state: &mut LuaState) -> LuaResult<u32> {
    let alpha = number_arg(state, 2).unwrap_or(0.0);
    with_visual_frame(state, |frame| frame.task_blob_outside.alpha = alpha)
}
pub fn set_task_blob_ring_texture(state: &mut LuaState) -> LuaResult<u32> {
    let asset = texture_arg(state, 2);
    with_visual_frame(state, |frame| frame.task_blob_ring.texture = asset)
}
pub fn set_task_blob_ring_alpha(state: &mut LuaState) -> LuaResult<u32> {
    let alpha = number_arg(state, 2).unwrap_or(0.0);
    with_visual_frame(state, |frame| frame.task_blob_ring.alpha = alpha)
}
pub fn set_task_blob_ring_scalar(state: &mut LuaState) -> LuaResult<u32> {
    let scalar = number_arg(state, 2).unwrap_or(0.0);
    with_visual_frame(state, |frame| frame.task_blob_ring.scalar = scalar)
}
pub fn set_arch_blob_inside_texture(state: &mut LuaState) -> LuaResult<u32> {
    let asset = texture_arg(state, 2);
    with_visual_frame(state, |frame| frame.arch_blob_inside.texture = asset)
}
pub fn set_arch_blob_inside_alpha(state: &mut LuaState) -> LuaResult<u32> {
    let alpha = number_arg(state, 2).unwrap_or(0.0);
    with_visual_frame(state, |frame| frame.arch_blob_inside.alpha = alpha)
}
pub fn set_arch_blob_outside_texture(state: &mut LuaState) -> LuaResult<u32> {
    let asset = texture_arg(state, 2);
    with_visual_frame(state, |frame| frame.arch_blob_outside.texture = asset)
}
pub fn set_arch_blob_outside_alpha(state: &mut LuaState) -> LuaResult<u32> {
    let alpha = number_arg(state, 2).unwrap_or(0.0);
    with_visual_frame(state, |frame| frame.arch_blob_outside.alpha = alpha)
}
pub fn set_arch_blob_ring_texture(state: &mut LuaState) -> LuaResult<u32> {
    let asset = texture_arg(state, 2);
    with_visual_frame(state, |frame| frame.arch_blob_ring.texture = asset)
}
pub fn set_arch_blob_ring_alpha(state: &mut LuaState) -> LuaResult<u32> {
    let alpha = number_arg(state, 2).unwrap_or(0.0);
    with_visual_frame(state, |frame| frame.arch_blob_ring.alpha = alpha)
}
pub fn set_arch_blob_ring_scalar(state: &mut LuaState) -> LuaResult<u32> {
    let scalar = number_arg(state, 2).unwrap_or(0.0);
    with_visual_frame(state, |frame| frame.arch_blob_ring.scalar = scalar)
}
...
pub fn set_player_ping_texture(state: &mut LuaState) -> LuaResult<u32> {
    let id = frame_id_from_stack(state, 1)?;
    let texture_type = number_arg(state, 2).unwrap_or(0.0) as i32;
    let asset = texture_arg(state, 3);
    let width = number_arg(state, 4).unwrap_or(0.0);
    let height = number_arg(state, 5).unwrap_or(0.0);
    let mut sim = borrow_state_mut(state)?;
...
pub fn set_player_ping_scale(state: &mut LuaState) -> LuaResult<u32> {
    let id = frame_id_from_stack(state, 1)?;
    let scale = number_arg(state, 2).unwrap_or(1.0);
    let mut sim = borrow_state_mut(state)?;
    unit_position_state_mut(&mut sim, id).player_ping_scale = scale;
    Ok(0)
}

pub fn get_player_ping_scale(state: &mut LuaState) -> LuaResult<u32> {
    let id = frame_id_from_stack(state, 1)?;
    let sim = borrow_state(state)?;
    let scale = sim
        .unit_position_frames
...
pub fn start_player_ping(state: &mut LuaState) -> LuaResult<u32> {
    let id = frame_id_from_stack(state, 1)?;
    let duration = number_arg(state, 2).unwrap_or(0.0);
    let fade_duration = number_arg(state, 3).unwrap_or(0.0);
    let mut sim = borrow_state_mut(state)?;
...
pub fn stop_player_ping(state: &mut LuaState) -> LuaResult<u32> {
    let id = frame_id_from_stack(state, 1)?;
    let mut sim = borrow_state_mut(state)?;
...
pub fn add_unit(state: &mut LuaState) -> LuaResult<u32> {
    let id = frame_id_from_stack(state, 1)?;
    let Some(unit) = texture_arg(state, 2) else {
        return Ok(0);
    };
    let asset = texture_arg(state, 3);
    let width = number_arg(state, 4);
    let height = number_arg(state, 5);
    let color = match (
        number_arg(state, 6),
        number_arg(state, 7),
        number_arg(state, 8),
    ...
    let sublevel = number_arg(state, 10).map(|value| value as i32);
    let show_facing = bool_arg_opt(state, 11);
    let mut sim = borrow_state_mut(state)?;
    let unit_state = unit_position_state_mut(&mut sim, id);
    unit_state
        .units
        .push(crate::lua_api::state::UnitPositionUnit {
            unit,
            asset,
            width,
            height,
...
pub fn finalize_units(state: &mut LuaState) -> LuaResult<u32> {
    let id = frame_id_from_stack(state, 1)?;
    let mut sim = borrow_state_mut(state)?;
...
pub fn set_unit_color(state: &mut LuaState) -> LuaResult<u32> {
    let id = frame_id_from_stack(state, 1)?;
    let Some(unit) = texture_arg(state, 2) else {
        return Ok(0);
    };
    let Some(color) = (match (
        number_arg(state, 3),
        number_arg(state, 4),
        number_arg(state, 5),
        number_arg(state, 6),
    ...
    for entry in &mut unit_state.units {
        if entry.unit == unit {
...
pub fn get_mouse_over_units(state: &mut LuaState) -> LuaResult<u32> {
    let id = frame_id_from_stack(state, 1)?;
    let sim = borrow_state(state)?;
    let units = sim
        .unit_position_frames
...
pub fn clear_units(state: &mut LuaState) -> LuaResult<u32> {
    let id = frame_id_from_stack(state, 1)?;
    let mut sim = borrow_state_mut(state)?;
    let unit_state = unit_position_state_mut(&mut sim, id);
...
pub fn set_fog_of_war_background_atlas(state: &mut LuaState) -> LuaResult<u32> {
    let id = frame_id_from_stack(state, 1)?;
    let atlas = texture_arg(state, 2);
    let mut sim = borrow_state_mut(state)?;
...
pub fn get_fog_of_war_background_atlas(state: &mut LuaState) -> LuaResult<u32> {
    let id = frame_id_from_stack(state, 1)?;
    let sim = borrow_state(state)?;
    let atlas = sim
        .fog_of_war_frames
        .get(&id)
        .and_then(|fog| fog.background_atlas.clone());
    drop(sim);
    match atlas {
        Some(value) => {
...
pub fn set_fog_of_war_mask_atlas(state: &mut LuaState) -> LuaResult<u32> {
    let id = frame_id_from_stack(state, 1)?;
    let atlas = texture_arg(state, 2);
    let mut sim = borrow_state_mut(state)?;
    let fog = sim.fog_of_war_frames.entry(id).or_default();
...
pub fn get_fog_of_war_mask_atlas(state: &mut LuaState) -> LuaResult<u32> {
    let id = frame_id_from_stack(state, 1)?;
    let sim = borrow_state(state)?;
    let atlas = sim
        .fog_of_war_frames
        .get(&id)
        .and_then(|fog| fog.mask_atlas.clone());
    drop(sim);
    match atlas {
        Some(value) => {
            let value = create_string(state, &value);
...
pub fn set_fog_of_war_mask_scalar(state: &mut LuaState) -> LuaResult<u32> {
    let id = frame_id_from_stack(state, 1)?;
    let scalar = number_arg(state, 2);
    let mut sim = borrow_state_mut(state)?;
    let fog = sim.fog_of_war_frames.entry(id).or_default();
...
pub fn get_fog_of_war_mask_scalar(state: &mut LuaState) -> LuaResult<u32> {
    let id = frame_id_from_stack(state, 1)?;
    let sim = borrow_state(state)?;
    let scalar = sim
        .fog_of_war_frames
...
pub fn draw_none(state: &mut LuaState) -> LuaResult<u32> {
    let id = frame_id_from_stack(state, 1)?;
    let mut sim = borrow_state_mut(state)?;
    sim.quest_blobs.entry(id).or_default().active_quests.clear();
    Ok(0)
}
