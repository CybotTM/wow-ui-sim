...
pub(super) fn attribute_to_val(
    ...
    match attr {
        ...
        Some(crate::widget::AttributeValue::String(s)) => {
            crate::lua_api::methods::create_string(state, s)
        }
        Some(crate::widget::AttributeValue::LuaRef(key)) => {
            match script_helpers::registry_table(state, ATTR_REFS_KEY) {
                Some(table) => script_helpers::table_get_str(state, table, key),
                None => Val::Nil,
            }
        }
    }
...
pub(super) fn val_to_attribute(
    ...
    match val {
        ...
        _ => {
            ...
            let refs = script_helpers::registry_table_or_create(state, ATTR_REFS_KEY);
            script_helpers::table_set_str(state, refs, key, val);
