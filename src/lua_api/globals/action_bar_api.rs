...
pub(crate) fn slot_from_value(v: &Value) -> Option<u32> {
    match v {
...
pub(crate) fn action_texture_path(state: &SimState, slot: u32) -> Option<String> {
...
pub fn register_c_action_bar_namespace(lua: &Lua, state: Rc<RefCell<SimState>>) -> Result<()> {
    super::action_bar_api_namespace::register_c_action_bar_namespace(lua, state)
}
