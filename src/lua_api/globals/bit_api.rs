...
fn bit_arshift_fn(lua: &Lua) -> Result<mlua::Function> {
    lua.create_function(|_, (a, n): (mlua::Number, mlua::Number)| {
        let shift = to_u32(n);
        if shift >= 32 {
            return Ok(if (to_u32(a) as i32) < 0 {
                0xFFFFFFFFu32
            } else {
                0u32
            });
