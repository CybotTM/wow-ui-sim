...
pub(super) fn transmog_category_info(
    category_id: i32,
...
pub(super) fn add_i32_stub_with_arg<A: mlua::FromLuaMulti>(
    lua: &Lua,
    t: &mlua::Table,
    name: &str,
    value: i32,
...
pub(super) fn add_bool_stub_with_arg<A: mlua::FromLuaMulti>(
    lua: &Lua,
    t: &mlua::Table,
    name: &str,
    value: bool,
...
pub(super) fn add_table_stub_with_arg<A: mlua::FromLuaMulti>(
    lua: &Lua,
    t: &mlua::Table,
    name: &str,
) -> Result<()> {
...
pub(super) fn add_nil_stub_with_arg<A: mlua::FromLuaMulti>(
    lua: &Lua,
    t: &mlua::Table,
    name: &str,
) -> Result<()> {
...
fn register_get_mount_info_by_id(
    ...
    t.set(
        "GetMountInfoByID",
        lua.create_function(move |lua, mount_id: u32| {
            let st = state.borrow();
    ...
    t.set(
        ...
        lua.create_function(move |lua, mount_id: u32| {
            ...
            Ok(mlua::MultiValue::from_vec(vec![
                Value::Integer(0),
                Value::String(lua.create_string("")?),
                Value::String(lua.create_string("Drop")?),
                Value::Boolean(false),
                Value::Integer(m.mount_type as i64),
                Value::Integer(0),
                Value::Integer(0),
                Value::Integer(0),
                Value::Boolean(false),
...
fn register_mount_filter_methods(lua: &Lua, t: &mlua::Table) -> Result<()> {
    t.set(
        ...
        lua.create_function(|_, _: i32| Ok(true))?,
    )?;
    t.set(
        "SetCollectedFilterSetting",
        lua.create_function(|_, (_, _): (i32, bool)| Ok(()))?,
    )?;
    t.set(
        "GetIsFavorite",
        lua.create_function(|_, _: i32| Ok((false, false)))?,
    )?;
    t.set(
        "SetIsFavorite",
        lua.create_function(|_, (_, _): (i32, bool)| Ok(()))?,
