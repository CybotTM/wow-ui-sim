...
//! - C_CovenantSanctumUI, C_WeeklyRewards, C_ContributionCollector, C_Scenario
...
//!
//! Housing namespaces (C_Housing, C_HousingCatalog, etc.) are in `c_misc_api_ui_housing`.
...
pub(super) fn register_all(lua: &Lua, state: Rc<RefCell<crate::lua_api::SimState>>) -> Result<()> {
    ...
    super::c_misc_api_ui_housing::register_c_housing(lua)?;
...
fn register_c_game_rules(lua: &Lua) -> Result<()> {
    ...
    t.set("IsHardcoreActive", lua.create_function(|_, ()| Ok(false))?)?;
