...
fn register_entrance_getters(lua: &Lua, t: &mlua::Table) -> Result<()> {
    ...
    t.set(
        "GetActiveDelveTier",
        lua.create_function(move |_, ()| s.get::<mlua::Table>("__activeDelveTier"))?,
    )?;
    let s = t.clone();
    t.set(
        "GetDelveEntranceBackgroundWidgetSetID",
        lua.create_function(move |_, ()| s.get::<i32>("__delveEntranceBackgroundWidgetSetID"))?,
    )?;
    let s = t.clone();
    t.set(
        "GetDelveEntranceDescriptionString",
        lua.create_function(move |_, ()| s.get::<String>("__delveEntranceDescriptionString"))?,
    )?;
    let s = t.clone();
    t.set(
        "GetDelveEntranceHeaderString",
        lua.create_function(move |_, ()| s.get::<String>("__delveEntranceHeaderString"))?,
    )?;
    let s = t.clone();
    t.set(
        "GetDelveEntranceMapID",
        lua.create_function(move |_, ()| s.get::<i32>("__delveEntranceMapID"))?,
    )?;
    let s = t.clone();
    t.set(
        "GetDelveEntranceTiers",
        lua.create_function(move |_, ()| s.get::<mlua::Table>("__delveEntranceTiers"))?,
    )?;
    Ok(())
}
...
fn register_affix_methods(lua: &Lua, t: &mlua::Table) -> Result<()> {
    ...
    t.set(
        ...
        lua.create_function(move |_, ()| s.get::<i32>("__tieredEntranceOptionalAffixTraitTreeID"))?,
    )?;
    Ok(())
}
