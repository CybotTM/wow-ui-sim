...
//! - C_CatalogShop, C_Who, C_PrivateAuras
...
pub(super) fn register_shop_who_auras(lua: &Lua, g: &mlua::Table) -> Result<()> {
...
fn first_category_by_product(lua: &Lua, product_id: i64) -> Result<Value> {
...
fn refresh_virtual_currency_balance(lua: &Lua, currency_code: String) -> Result<()> {
    fire_event(
        lua,
...
pub(super) fn fire_event(lua: &Lua, event_name: &str, args: &[Value]) -> Result<()> {
