...
#[path = "parser_global_tooltip.rs"]
mod parser_global_tooltip;

pub(super) use self::parser_global_dispatch::parse_global_family;
pub(super) use self::parser_global_tooltip::{
    parse_conditional_tooltip, parse_global_tooltip_set_owner_then_set_text,
    parse_global_tooltip_set_owner_then_set_text_literal,
};
...
fn parse_inline_global_method_with_self_arg(stmt: &str) -> Option<(&str, &str)> {
    parse_inline_global_method_with_exact_args(stmt, "self")
}

fn parse_inline_global_method_with_exact_args<'a>(
    stmt: &'a str,
    expected_args: &str,
) -> Option<(&'a str, &'a str)> {
    let (target_path, remainder) = stmt.rsplit_once(':')?;
    ...
    let target_path = target_path.trim();
    let method_name = method_name.trim();
    (is_fast_handler_path(target_path) && is_fast_identifier(method_name) && args == expected_args)
        .then_some((target_path, method_name))
}
...
fn parse_inline_global_method_with_self_id_arg(stmt: &str) -> Option<(&str, &str)> {
    parse_inline_global_method_with_exact_args(stmt, "self:GetID()")
}
