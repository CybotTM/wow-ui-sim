...
pub fn register_mixin_api(lua: &Lua) -> Result<()> {
    ...
    globals.set(
        ...
        register_scrolling_message_frame_mixin(lua)?,
    )?;
...
fn register_poi_button_mixin(lua: &Lua) -> Result<mlua::Table> {
    ...
    t.set(
        "OnClick",
        lua.create_function(|_, (_self, _button): (Value, Option<String>)| Ok(()))?,
    )?;
    t.set(
        "UpdateButtonStyle",
    ...
    t.set(
        "SetSelected",
        lua.create_function(|_, (_self, _selected): (Value, bool)| Ok(()))?,
    )?;
    t.set(
        "GetSelected",
...
fn add_taggable_methods(lua: &Lua, t: &mlua::Table) -> Result<()> {
    t.set(
        "AddTag",
        lua.create_function(|_, (_self, _tag): (Value, i32)| Ok(()))?,
    )?;
    t.set(
        "RemoveTag",
        lua.create_function(|_, (_self, _tag): (Value, i32)| Ok(()))?,
    ...
    t.set(
        "MatchesTag",
        lua.create_function(|_, (_self, _tag): (Value, i32)| Ok(false))?,
    )?;
    t.set(
        "MatchesAnyTag",
        lua.create_function(|_, _args: mlua::Variadic<Value>| Ok(false))?,
    )?;
    t.set(
        "MatchesAllTags",
        lua.create_function(|_, _args: mlua::Variadic<Value>| Ok(false))?,
...
fn register_map_canvas_pin_mixin(lua: &Lua) -> Result<mlua::Table> {
    ...
    t.set(
        ...
        lua.create_function(|_, (_self, _button): (Value, String)| Ok(false))?,
    )?;
    t.set(
...
fn add_pin_nudge_methods(lua: &Lua, t: &mlua::Table) -> Result<()> {
    t.set(
        ...
        lua.create_function(|_, (_self, _radius): (Value, f64)| Ok(()))?,
    )?;
    t.set(
    ...
    t.set(
        ...
        lua.create_function(|_, (_self, _factor): (Value, f64)| Ok(()))?,
    )?;
    t.set(
...
fn register_menu(lua: &Lua) -> Result<mlua::Table> {
    ...
    menu.set(
        "ModifyMenu",
        lua.create_function(|_, (_owner, _generator_fn): (Value, mlua::Function)| Ok(()))?,
    )?;
