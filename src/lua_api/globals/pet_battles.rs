...
pub fn get_battle_state(state: &mut LuaState) -> LuaResult<u32> {
    let s = borrow_state(state)?.pet_battles.battle_state;
...
pub fn register_all(lua: &mut rilua::Lua) -> LuaResult<()> {
    ...
    table_set_rust_fn(state, table_ref, "GetNumPets", get_num_pets)?;
    table_set_rust_fn(state, table_ref, "GetBattleState", get_battle_state)?;
    Ok(())
}
...
pub fn admin_set_pet_battle_counts(state: &mut LuaState) -> LuaResult<u32> {
    ...
    st.pet_battles.num_pets_player = player.max(0);
    st.pet_battles.num_pets_enemy = enemy.max(0);
    Ok(0)
}
...
pub fn admin_set_pet_battle_state(state: &mut LuaState) -> LuaResult<u32> {
    ...
    borrow_state_mut(state)?.pet_battles.battle_state = s;
    Ok(0)
}
