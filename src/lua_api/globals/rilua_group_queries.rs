...
//!
//! Called once from `rilua_admin::register_all` after the A_Admin table
//! is installed, so the globals are in place before any addon script runs.
//!
//! Registering these here keeps the rilua_admin module focused on the
...
fn string_join(state: &mut LuaState) -> LuaResult<u32> {
    super::rilua_utility_system_spell::strjoin(state)
}
