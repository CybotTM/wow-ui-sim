...
fn register_melee_stats(lua: &Lua) -> Result<()> {
    ...
    g.set(
        "UnitArmor",
        lua.create_function(|_, _unit: Value| Ok((0i32, 0i32, 0i32, 0i32, 0i32)))?,
    )?;
    // UnitDamage(unit) -> minDmg, maxDmg, minOff, maxOff, posPhys, negPhys, pct
    g.set(
        "UnitDamage",
        lua.create_function(|_, _unit: Value| {
            Ok((
                0.0_f64, 0.0_f64, 0.0_f64, 0.0_f64, 0.0_f64, 0.0_f64, 100.0_f64,
            ))
        })?,
    )?;
    ...
    g.set(
        "UnitAttackPower",
        lua.create_function(|_, _unit: Value| Ok((0i32, 0i32, 0i32)))?,
    )?;
    ...
    g.set(
        "UnitAttackSpeed",
        lua.create_function(|_, _unit: Value| Ok((2.0_f64, 2.0_f64)))?,
    )?;
    Ok(())
}
...
fn register_ranged_and_form_stats(lua: &Lua) -> Result<()> {
    ...
    g.set(
        ...
        lua.create_function(|_, _idx: Value| Ok((Value::Nil, false, false, 0i32)))?,
    )?;
    g.set(
        "GetShapeshiftFormID",
    ...
    g.set(
        "GetPetActionInfo",
        lua.create_function(|_, _idx: Value| {
    ...
    g.set(
        "IsPetAttackAction",
        lua.create_function(|_, _idx: Value| Ok(false))?,
    )?;
    g.set(
        "GetPetActionCooldown",
        lua.create_function(|_, _idx: Value| Ok((0.0_f64, 0.0_f64, true)))?,
    )?;
    // Battle pet numeric stubs (no battle pets in sim)
    g.set(
        "UnitBattlePetLevel",
        lua.create_function(|_, _unit: Value| Ok(0))?,
    )?;
    g.set(
        "UnitBattlePetType",
