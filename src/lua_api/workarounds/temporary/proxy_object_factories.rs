...
if rawget(C_StringUtil, "CreateSecondsFormatter") == nil then
  ...
  -- The previous six methods left SetRounding missing, which aborted
  -- Blizzard_AuraContainer/Blizzard_AuraContainerShared.lua at line 94 and cost
  -- the whole file. Setters store, getters read back, predicates read the same
  -- field; only Format produces text.
