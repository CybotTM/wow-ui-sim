local function __wow_hide_namespace_key(tableValue, key)
  ...
  local removedKeys = rawget(tableValue, "__wow_ptr_removed_keys")
  if type(removedKeys) ~= "table" then
    removedKeys = {}
    rawset(tableValue, "__wow_ptr_removed_keys", removedKeys)
  end
  removedKeys[key] = true

  local mt = getmetatable(tableValue) or {}
  if rawget(mt, "__wow_ptr_removed_filter") then
    return
  end

  local oldIndex = mt.__index
  mt.__index = function(t, lookupKey)
    local removed = rawget(t, "__wow_ptr_removed_keys")
  ...
  mt.__wow_ptr_removed_filter = true
...
local __wow_ptr_removed_cvars = {
  lastlockeddelvescompanionabilities = true,
  slugsupersampling = true,
}

local function __wow_is_removed_cvar(name)
  return type(name) == "string" and __wow_ptr_removed_cvars[string.lower(name)] == true
end

if type(GetCVar) == "function" and rawget(_G, "__wow_ptr_original_GetCVar") == nil then
  rawset(_G, "__wow_ptr_original_GetCVar", GetCVar)
  function GetCVar(name)
    if __wow_is_removed_cvar(name) then
      return nil
    end
    return __wow_ptr_original_GetCVar(name)
  end
end

if type(GetCVarDefault) == "function" and rawget(_G, "__wow_ptr_original_GetCVarDefault") == nil then
  rawset(_G, "__wow_ptr_original_GetCVarDefault", GetCVarDefault)
  function GetCVarDefault(name)
    if __wow_is_removed_cvar(name) then
      return nil
    end
    return __wow_ptr_original_GetCVarDefault(name)
  end
end

if type(C_CVar) == "table" and type(C_CVar.GetCVar) == "function" and rawget(C_CVar, "__wow_ptr_original_GetCVar") == nil then
  rawset(C_CVar, "__wow_ptr_original_GetCVar", C_CVar.GetCVar)
  function C_CVar.GetCVar(name)
    if __wow_is_removed_cvar(name) then
      return nil
    end
    return C_CVar.__wow_ptr_original_GetCVar(name)
  end
end
