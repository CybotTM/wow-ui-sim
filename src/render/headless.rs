...
mod tests {
    ...
    fn atlas_sampler_interpolates_between_texels() {
        ...
        let (device, queue) = super::create_headless_device();
        let image = super::render_headless_primitive_to_image(&mut primitive, &device, &queue, 16, 16);
    ...
    fn single_texel_strip_stretched_wide_reads_back_constant_row() {
        ...
        let (device, queue) = super::create_headless_device();
        let image = super::render_headless_primitive_to_image(
            &mut primitive,
            &device,
            &queue,
            width,
