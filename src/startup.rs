...
mod seed_buff_duration_tests {
    ...
    fn permanent_auras_get_no_duration_text() {
        ...
        assert_eq!(timed, "positive", "an aura with a real duration keeps its timer");
        assert_eq!(
            permanent, "none",
            "a permanent aura must get no timer text; a negative countdown is the defect"
        );
        assert_eq!(no_info, "none", "an aura with no buttonInfo must be skipped");
        assert_eq!(
            zero_duration, "none",
