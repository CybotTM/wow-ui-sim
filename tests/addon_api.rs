...
fn test_get_addon_dependencies_by_index() {
    ...
    let (count, first): (i32, String) = env
        .eval(
            r#"
            -- Index 3 is MultiDepAddon (1=__BuiltIn, 2=ParentAddon, 3=ChildAddon, 4=MultiDepAddon)
            local idx
...
fn test_get_addon_dependencies_flows_into_variadic_callee() {
    // Mirrors AddonTooltip_Update's call site: the result feeds directly
    // into another function as varargs, where each dep becomes a positional
    // argument to AddonTooltip_BuildDeps(...).
    let env = env_with_dep_addons();
...
fn test_get_addon_dependencies_walks_all_loaded_state() {
    // Capturing the variadic into a table lets a caller walk every dep
    // (the canonical pattern for full LOD readiness).
    let env = env_with_dep_addons();
    {
        let mut sim = env.state().borrow_mut();
        for addon in sim.addons.iter_mut() {
            if addon.folder_name == "ParentAddon"
                || addon.folder_name == "ChildAddon"
                || addon.folder_name == "Sibling"
            {
                addon.loaded = true;
            }
        }
    }
    let all_loaded: bool = env
        .eval(
            r#"
...

    {
        ...
        if let Some(a) = sim
            .addons
            .iter_mut()
            .find(|a| a.folder_name == "ChildAddon")
        {
            a.loaded = false;
        }
    }
    let still_loaded: bool = env
        .eval(
            r#"
            local deps = { C_AddOns.GetAddOnDependencies('MultiDepAddon') }
            for _, dep in ipairs(deps) do
