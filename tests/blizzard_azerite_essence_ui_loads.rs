...
#[test]
fn blizzard_azerite_essence_ui_lod_loads_after_colors() {
    common::with_perf_lock(|| {
        common::with_timeout(240, || {
            with_blizzard_addon_smoke_shape(&[], &[], |env, _loaded| {
                ...
                let (dependency_loaded, dependency_reason): (bool, Option<String>) = env
                    .eval(r#"return C_AddOns.LoadAddOn("Blizzard_Colors")"#)
                    .expect("C_AddOns.LoadAddOn should return for Blizzard_Colors");
                assert!(
                    dependency_loaded,
                    "`{DEPENDENCY}` should load before `{ROOT}`: {dependency_reason:?}"
                );
                ...
                let is_loaded: bool = env
                    .eval(r#"return C_AddOns.IsAddOnLoaded("Blizzard_AzeriteEssenceUI")"#)
