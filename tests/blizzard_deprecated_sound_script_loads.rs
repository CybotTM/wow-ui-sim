...
fn blizzard_deprecated_sound_script_toc_is_minimal_with_no_flags_or_deps() {
    ...
    assert!(
        ...
        "Blizzard_DeprecatedSoundScript declares NO dependencies — the single shim simply \
         forwards to C_Sound, which is bootstrapped at env init via runtime_surface_\
         bootstrap.lua:6341 (`C_Sound = C_Sound or __wow_namespace()` — created with \
         __wow_namespace_mt attached)"
    );
...
#[test]
fn blizzard_deprecated_sound_script_loads_without_errors() {
...
#[test]
fn blizzard_deprecated_sound_script_play_vocal_error_sound_id_aliases_play_vocal_error_sound() {
    let env = load_full_game_ui();
    ...
    assert_eq!(
        ...
        "Deprecated_SoundScript.lua line 9 reads `PlayVocalErrorSoundID = \
         ...
         (without the `ID` suffix). C_Sound.PlayVocalErrorSound is NOT explicitly stubbed; \
         it resolves via `__wow_namespace_mt.__index` (runtime_surface_bootstrap.lua:2019-\
         2028) to a no-op `function() return nil end` closure"
    );
    ...
    assert!(
        ...
        "PlayVocalErrorSoundID must be IDENTITY-EQUAL to C_Sound.PlayVocalErrorSound — \
         both sides see the SAME no-op closure (the namespace `__index` rawset-caches the \
         closure on first read so subsequent reads return the same function value)"
    );
}

#[test]
fn blizzard_deprecated_sound_script_play_vocal_error_sound_id_calls_without_error() {
    let env = load_full_game_ui();
    ...
    assert!(
        ...
        "Calling PlayVocalErrorSoundID(0) must NOT throw — it resolves through the \
         namespace `__index` to a no-op closure `function() return nil end`. pcall \
         returns ok=true"
    );
}

#[test]
fn blizzard_deprecated_sound_script_load_deprecation_fallbacks_cvar_is_default_on() {
    let env = load_full_game_ui();
