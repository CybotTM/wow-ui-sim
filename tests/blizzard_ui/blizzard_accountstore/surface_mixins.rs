...
//! Two tests pin both halves of the contract:
//!
//! - `account_store_item_display_mixin_methods_match_actual_source`
//!   walks the five PLAN-named methods that ARE present (`OnLoad`,
//!   `OnShow`, `OnHide`, `OnCategorySelected`, `SetPage`) plus the
//!   seven architecturally-critical PLAN-omitted-but-present methods
//!   (`InitializeStore`, `OnEvent`, `OnMouseWheel`, `OnStoreFrontSet`,
//!   `GetMaxPage`, `UpdateCurrencyAvailable`, `CreateItemRack`),
//!   asserting each is a `function`. The PLAN-omitted assertions
//!   guard against silent removal of the dispatch / state-init /
//!   page-clamping surface — without them the item-display contract
//!   would silently regress (e.g. dropping `OnStoreFrontSet` would
//!   leave the EventRegistry callback registered but with no handler,
//!   so `AccountStoreMixin:SetStoreFrontID` would fire the event and
//!   nothing would re-initialize the per-store category state).
//!
//! - `account_store_item_display_mixin_does_not_define_plan_named_methods_that_are_actually_absent`
//!   asserts `RefreshSelectedCard` is nil on
//!   `AccountStoreItemDisplayMixin`. This is the spec/source mismatch
//!   tripwire — if Blizzard adds `RefreshSelectedCard` (or renames
//!   `UpdateCurrencyAvailable` / `SetPage` to match), the test flips
//!   and forces a re-pin against the new refresh-chain contract.
