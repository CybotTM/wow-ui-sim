...
fn seed_bitfield_state(env: &WowLuaEnv) {
    env.exec(
        r#"
        TEST_INITIAL_EDITMODE = GetCVarBitfield("closedInfoFramesAccountWide", 3)
        SetCVarBitfield("closedInfoFramesAccountWide", 3, true)
        TEST_AFTER_SET = GetCVarBitfield("closedInfoFramesAccountWide", 3)
        TEST_RAW_VALUE = GetCVar("closedInfoFramesAccountWide")
...
fn cvar_bitfields_can_be_set_and_cleared() {
    ...
    let initial: bool = env.eval("return TEST_INITIAL_EDITMODE").unwrap();
    let after_set: bool = env.eval("return TEST_AFTER_SET").unwrap();
    let raw_value: String = env.eval("return TEST_RAW_VALUE").unwrap();
    let after_clear: bool = env.eval("return TEST_AFTER_CLEAR").unwrap();

    assert!(!initial);
    assert!(after_set);
    assert_eq!(raw_value, "8");
    assert!(!after_clear);
}
