...
fn settings_initializer_hook_is_temporary_workaround_not_runtime_bootstrap() {
    let runtime = fs::read_to_string("src/lua_api/env_init/runtime_surface_bootstrap.lua")
        .expect("read runtime bootstrap source");
    let settings_workaround =
        fs::read_to_string("src/lua_api/workarounds/temporary/settings_surface_defaults.rs")
