...
#[test]
fn test_create_frame_from_xml_inline_global_method_with_runtime_args_runs() {
    clear_templates();
    let env = WowLuaEnv::new().unwrap();
    env.exec(
        r#"
        ...
        _G.XmlInlineGlobalMethodLiteralTarget = {}
        function _G.XmlInlineGlobalMethodLiteralTarget:SetText(text, r, g, b, extra, flag)
            self.text = text
            self.r = r
            self.g = g
            self.b = b
...
#[test]
fn test_create_frame_from_xml_inline_global_method_sequence_with_runtime_args_runs() {
    clear_templates();
    let env = WowLuaEnv::new().unwrap();
    env.exec(
        r#"
        _G.ARTIFACT_XP_REWARD = "Artifact XP"
        _G.HIGHLIGHT_FONT_COLOR = { r = 0.1, g = 0.2, b = 0.3 }
        _G.XmlInlineTooltipTarget = {}
        function _G.XmlInlineTooltipTarget:SetOwner(frame, anchor)
            self.owner = frame
            self.anchor = anchor
        end
        function _G.XmlInlineTooltipTarget:SetText(text, r, g, b, extra, flag)
            self.text = text
            self.r = r
            self.g = g
            self.b = b
    ...
    let result: (String, String, f64, f64, f64, bool, bool) = env
        .eval(
            r#"
            return XmlInlineTooltipTarget.anchor,
                   XmlInlineTooltipTarget.text,
